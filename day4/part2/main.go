...
func main() {
	...
	guardID, minute, count := getMostFrequent(guardDataMap)

	fmt.Println("guard", guardID)
	fmt.Println("minute", minute, "count", count)
...
func getMostFrequent(guardDataMap map[int]*guardData) (int, int, int) {
	count := 0
	var guardID int
	var minute int
	for id, data := range guardDataMap {
		m, c := getMax(data.minuteCounts)
		if c > count {
			count = c
			guardID = id
			minute = m
