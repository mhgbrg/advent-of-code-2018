...
func (g group) String() string {
	...
	return fmt.Sprintf("%s %d (%d units)", teamStr, g.id, g.units)
}

func main() {
	start := time.Now()

	originalImmuneSystem, originalInfection := readInput()

	// With boost < 46 the immune system doesn't have enough attack power to kill
	// off the last infection, and so the fight goes on forever.
	for boost := 46; ; boost++ {
		immuneSystem, infection := make(map[*group]bool), make(map[*group]bool)
		for g := range originalImmuneSystem {
			boosted := &group{
				g.id,
				g.team,
				g.units,
			...
			immuneSystem[boosted] = true
		}
		for g := range originalInfection {
			copy := &group{
				g.id,
				g.team,
				g.units,
